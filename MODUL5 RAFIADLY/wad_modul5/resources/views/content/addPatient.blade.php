...
<body>
    ...
    <main class="container shadow d-flex flex-column p-5 mt-1">
        <div class="display-6 text-center"><b>Register {{ $vaccine->name }} Patient</b></div>
        <div class="container">
            <form method="POST" action="{{ route('uploadPatient') }}" enctype="multipart/form-data">
                @csrf

                <!-- <label for="" class="form-text"><b>Vaccine Id</b></label> -->
                <input type="text" class="form-control" name="idVaksin" value="{{ $vaccine->id }}" hidden id="">

                <label for="" class="form-text"><b>Vaccine Name</b></label>
                <input type="text" class="form-control" name="namaVaksin" value="{{ $vaccine->name }}" readonly id="">
                ...
                <input type="submit" name="register" value="REGISTER" class="btn btn-primary mt-4 d-block mx-auto w-50">
            </form>
        </div>
        </div>
    </main>
